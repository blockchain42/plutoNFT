...
const contract = `${dirname}/../src/agatar.js`;
...
test(`agatar mint`, async t=> {
  ...
  const { creatorFacet, publicFacet } = await E(zoe).startInstance(installation,issuerKeywordRecord,{tokenName: 'TestToken', moneyIssuer: bldIssuer,pricePerItem});
  ...
  // const publicFacet = await E(zoe).getPublicFacet(instance);
    ...
    const buyerBoughtNftAmount = await E(nftIssuer).getAmountOf(
      buyersNft,
    );

    t.is(buyerBoughtNftAmount.value[0].uri,imageURI,'Buyer should have received the correct image',);
